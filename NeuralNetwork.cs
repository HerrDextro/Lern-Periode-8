...
namespace TicTacToe_Neuronics
{
    ...
    public class NeuralNetwork
    {
        ...
        public List<Neuron> HiddenLayer3 { get; set; }*/
        public List<Neuron> OutputLayer { get; set; }

        private const double MOMENTUM = 0.4; //trying to avoid catastrophic forgetting
        private Dictionary<Synapse, double> previousWeightChanges = new Dictionary<Synapse, double>();

        public NeuralNetwork(int inputNeurons, int hiddenNeurons1, /*int hiddenNeurons2, int hiddenNeurons3,*/ int outputNeurons)
        {
            ...
            for (int i = 0; i < hiddenNeurons3; i++)
                HiddenLayer2.Add(new Neuron());*/
        ...
        {
            string directoryPath = @"C:\Users\Neo\source\repos\TTT Neuronics V1\TTT Neuronics V1\profiles";
            if (!Directory.Exists(directoryPath))
            {
                Directory.CreateDirectory(directoryPath);
            }

            string filename = Path.Combine(directoryPath, $"profile_{profileName}.txt");
        ...
        public void Train(double[] inputs, double[] targetOutputs, double learningRate = 0.1)
        {
            ...
            // Calculate output layer deltas with regularization
            for (int i = 0; i < OutputLayer.Count; i++)
            {
                ...
                double regularization = 0.01 * neuron.InputSynapses.Sum(s => s.Weight * s.Weight);
                neuron.Delta = output * (1 - output) * (target - output) - regularization;
            }

            // Calculate hidden layer deltas with dropout
            void CalculateLayerDeltas(List<Neuron> layer)
            {
                foreach (var neuron in layer)
                {
                    // Apply dropout during training
                    if (Random.Shared.NextDouble() < 0.95) // 5% dropout rate
                    {
            ...
            void UpdateLayer(List<Neuron> layer)
            {
                foreach (var neuron in layer)
                {
                    ...
                    {
                        double weightChange = learningRate * synapse.InputNeuron.Output * synapse.OutputNeuron.Delta;

                        // Add momentum
                        if (previousWeightChanges.TryGetValue(synapse, out double previousChange))
                        {
                            weightChange += MOMENTUM * previousChange;
                        }

                        synapse.Weight += weightChange;
                        previousWeightChanges[synapse] = weightChange;
                    }
                }
            }
